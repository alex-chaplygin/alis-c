 ...
 * изображение
 */
void draw_setup(vec_t *origin, image_t *im, int x_flip, rectangle_t *blit)
{
...
#ifdef DEBUG
  printf("draw setup: rows = %d cols = %d vid_add = %d im_add = %d x_fl = %d src = %d dst = %d\n", num_rows, num_cols, video_add, image_add, x_flip, (int)(blit_src - (byte *)im), (int)(blit_dst - video_buffer));
#endif
}
...
void render_image(vec_t *origin, image_t *im, int x_flip, rectangle_t *blit)
{
  switch (im->type) {
  case IMAGE_FILL:
    draw_setup(origin, im, x_flip, blit);
